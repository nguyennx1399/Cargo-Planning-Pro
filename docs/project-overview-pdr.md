...
**Status:** Skeleton (phases 0–5 in progress). Structure, types, and stubs in place; real logic marked `TODO(phase-N)`.
...
3. Fix violations by moving/swapping containers
...
|---|---|---|---|
| NF1 | 3D render performance | 20k containers @ 60fps | InstancedMesh per size |
| NF2 | Validation latency | <100ms per check | debounce on edit |
...
| NF5 | Data consistency | Single source of truth | Backend models mirror frontend types |
...
| 1 | Open BAPLIE → see in 3D + 2D; select any container; 20k instances @ 60fps |
| 2 | Planner fixes plan by hand; violations shown real-time; export BAPLIE |
...
**Frontend:** React 19 + TypeScript + Vite 6 + React Three Fiber 9 + drei 10 + three.js + Zustand 5 + TanStack Query 5

**Backend:** Python 3.11 + FastAPI + Pydantic v2 + uvicorn; OR-Tools, NumPy (phase 4); pytest + httpx

**Data:** PostgreSQL (phase 2+); BAPLIE EDIFACT interchange; JSON API
...
|------|--------|-----------|
| Solver bugs → unsafe plans | Critical | Validation engine catches all violations before return; tester reviews all solver outputs |
